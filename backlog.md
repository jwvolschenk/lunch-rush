...
- [x] Replace SaveLoad.TOWER_UNLOCK_MILESTONES, CARD_UNLOCK_MILESTONES, and STARTING_GOLD_BONUS_MILESTONES from const Dictionary to const Array of sorted dictionaries — GDScript dict iteration order is non-deterministic so tower/card unlock order varies between runs. Acceptance: milestones iterate in deterministic sorted order (by wave number for gold bonus, by name for towers/cards); functionality unchanged; unlock detection logic still works correctly.
