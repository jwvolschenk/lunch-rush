...
- [ ] Fix Tower.gd _fire() using non-existent add_sibling() — Tower.gd line calls `add_sibling(projectile)` which does not exist in GDScript; replace with `add_child(projectile)` to fix tower attacks crashing at runtime
