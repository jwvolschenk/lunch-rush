...
- [x] Bugfix: CardSelection.gd line 103 uses `CravingType.food_type_to_int()` and `CravingType.NONE` but the file only preloads as `CRAVING_TYPE` (const on line 4). `CravingType` is undefined causing runtime crash. Fix: replace `CravingType.` with `CRAVING_TYPE.` on line 103.
