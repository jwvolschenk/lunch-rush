...
- [ ] Fix GameState._apply_room_modifier() to use wave base values for multiplier instead of accumulated state — `wave_gold_reward = int(wave_gold_reward * (1.0 + gold_bonus * 0.01))` multiplies against current (potentially already-modified) wave_gold_reward; should multiply against the wave's base value (from wave resource) instead. Similarly wave_enemy_hp *= and wave_enemy_speed *= compound incorrectly. Store wave base values before applying room modifiers. Acceptance: room modifiers apply multiplicatively against clean wave base values, not against accumulated modifier state; wave_gold_reward baseline resets between room selections.
