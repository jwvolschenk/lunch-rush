...
func _on_card_selected(card_data: Dictionary) -> void:
	...
	# After card selection, show room selector, then start next wave
...
func _show_card_selection() -> void:
	if card_selection:
		# Use pending cards from GameState if available (generated by WaveManager)
		var pending = GameState.get("pending_cards")
		if pending and pending.size() > 0:
			card_selection.show_cards(pending)
			return
